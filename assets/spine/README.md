...
Place one export folder per symbol id (`CH`, `LM`, `OR`, `GR`, `ST`, `S7`, `DM`, `CR`, `WD`) and the client-only performance blob (`BL` — green square).
...
### Green square (performance blob `BL`)

Client-only VFX — not in maths or books. Required tracks:

| Track | Use |
|-------|-----|
| `idle` | Loop while settled on the strip |
| `spin` | Loop during reel scroll |
| `land` | Once on impact → `idle` |
| `dissolve` | Once on pop before strip tumble |

Wire-up is in `SYMBOL_VISUAL.BL` (`js/pixi/symbols.js`). Replace `blob.png` with your authored green-square art (256×256 canvas); the repo ships a placeholder atlas until then.
